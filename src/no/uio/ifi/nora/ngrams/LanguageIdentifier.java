...
// Apated from the Apache Nutch project.
package no.uio.ifi.nora.ngrams;
...
import no.uio.ifi.nora.ngrams.NGramProfile.NGramEntry;
...
public class LanguageIdentifier {
  ...
  private int analyzeLength = 1000;
    ...
    try {
      ...
      while (alllanguages.hasMoreElements()) {
        ...
        InputStream is = this.getClass().getClassLoader().getResourceAsStream(
                "no/uio/ifi/nora/ngrams/" + lang + "."+NGramProfile.FILE_EXTENSION);
  ...
  public String identifyAndScore(StringBuilder content) {

	  NGramProfile local = new NGramProfile("localNgram", minLength, maxLength);
		StringBuilder text = content;
	    if ((analyzeLength > 0) && (content.length() > analyzeLength)) {
	        text = new StringBuilder().append(content);
	        text.setLength(analyzeLength);
	    ...
	    local.analyze(text);
	    Iterator<NGramEntry> iter = local.getSorted().iterator();
	    float topscore = Float.MIN_VALUE;
	    String lang = "";
	    HashMap<NGramProfile, Float> scores = new HashMap<NGramProfile, Float>();
	    NGramEntry searched = null;
